...
def update(client):
    ...
    for cp in range(playerCount):
        entBase = dbg.read_int(client+getOffset(config,'entityListOffset')+cp*0x10)
        if (entBase == 0x0):
            continue
...
def noFlash(clientState):
    ...
    while not end:
        ...
        if win32gui.GetForegroundWindow() == csgoWindow:
            if dbg.read_int(clientState + getOffset(config,'clientStateInGameOffset')) == 6:  # If the client is in game
                localPlayer = me.dwBase
                dbg.write_float(localPlayer + getOffset(config,'flashDurationOffset'), 0.0)  # flashduration set 0
    ...
    while not end:  # This function is threaded so might as well do this :>
        ...
        if win32gui.GetForegroundWindow() == csgoWindow:
            if dbg.read_int(clientState + getOffset(config,'clientStateInGameOffset')) == 6:  # If the client is in game
                ...
                crossHairID = dbg.read_int(localPlayer + getOffset(config,'crossHairIDOffset'))  # Get the Entity ID of the entity in crosshairs
                if crossHairID == 0:  # If no entity in crosshair
                    continue
                crossEntitypnt = dbg.read_int(client + getOffset(config,'entityListOffset') + ((crossHairID - 1) * 0x10))  # Find entity based on ID defined by crossHairID
                crossEntity = dbg.read_int(crossEntitypnt)  # Get the base of the entity in crosshair

                crossEntityTeam = dbg.read_int(crossEntity + getOffset(config,'teamNumOffset'))  # Get team of Entity in Crosshair

                if crossEntityTeam != 2 and crossEntityTeam != 3:  # If the entity is not a terrorist or counter-terrorist
                    continue

                crossEntityDormant = dbg.read_int(crossEntity + getOffset(config,'dormantOffset'))  # Get boolean that states whether entity in crosshair is dormant or not
                ...
                if  localPlayerTeam != crossEntityTeam and crossEntityDormant == 0:
                    ...
                    while crossHairID != 0:
                        crossHairID = dbg.read_int(localPlayer + getOffset(config,'crossHairIDOffset'))  # Re-get the crosshair ID to check if maybe no longer an entity in my crosshair
                        dbg.write_int(client + getOffset(config,'forceAttackOffset'), 5)  # Shoot
                        time.sleep(0.1)
                        dbg.write_int(client + getOffset(config,'forceAttackOffset'), 4)  # Stop shooting
...
def glowESP(client):
    glowPointer = dbg.read_int(client + getOffset(config,'glowObjectOffset'))  # Get the glow Pointer
    #playerCount = dbg.read_int(client + getOffset(config,'glowObjectOffset') + 0x4)
    playerCount = 64
    for i in range(1, playerCount):  # For each player until the max players available
        glowCurrentPlayer = dbg.read_int(client+getOffset(config,'entityListOffset')+((i-1)*16))
        ...
        player = players[i]
## burdan ekle
        glowCurrentPlayerGlowIndex = dbg.read_int(player.dwBase + getOffset(config,'glowIndexOffset'))  # Get the glowIndex of the glowCurrentPlayer entity
...
def BHOP(client, localPlayer, clientState):
    ...
    while not end:
        if not autoBHOPEnabled:
            break
        if win32gui.GetForegroundWindow() == csgoWindow and dbg.read_int(clientState + getOffset(config,'clientStateInGameOffset')) == 6:  # If client is in game
            flags = dbg.read_int(localPlayer + getOffset(config,'flagsOffset'))  # Get client flags
            if flags & (1 << 0) and win32api.GetAsyncKeyState(0x20):  # If localPlayer on the ground and if space is held
                dbg.write_int(client + getOffset(config,'forceJumpOffset'), 6)  # Autojump
            flags = dbg.read_int(localPlayer + getOffset(config,'flagsOffset'))  # Get the latest flags again
        time.sleep(0.01)
    ...
    while not end:
        ...
        if win32gui.GetForegroundWindow() == csgoWindow:
            ...
            for i in range(0, playerCount):
                ...
                entDormant = dbg.read_int(ent.dwBase + getOffset(config,'dormantOffset'))  # Get boolean that states whether glowCurrentPlayer entity is dormant or not
                if entDormant != 0:
                    continue

                myTeamID = dbg.read_int(localPlayer + getOffset(config,'teamNumOffset'))  # Get the team ID of the localPlayer
                entityBaseTeamID = dbg.read_int(ent.dwBase + getOffset(config,'teamNumOffset'))  # Get the team ID of the ent entity

                if entityBaseTeamID != 2 and entityBaseTeamID != 3:
                    continue

                localPlayerX = dbg.read_float(localPlayer + getOffset(config,'vecOriginOffset'))  # Get the X coordinate of the vecOrigin of the localPlayer
                localPlayerY = dbg.read_float(localPlayer + getOffset(config,'vecOriginOffset') + 0x4)  # Get the Y coordinate of the vecOrigin of the localPlayer
                localPlayerZ = dbg.read_float(localPlayer + getOffset(config,'vecOriginOffset') + 0x8)  # Get the Z coordinate of the vecOrigin of the localPlayer

                entityX = dbg.read_float(ent.dwBase + getOffset(config,'vecOriginOffset'))  # Get the X coordinate of the vecOrigin of the ent
                entityY = dbg.read_float(ent.dwBase + getOffset(config,'vecOriginOffset') + 0x4)  # Get the Y coordinate of the vecOrigin of the ent
                entityZ = dbg.read_float(ent.dwBase + getOffset(config,'vecOriginOffset') + 0x8)  # Get the Z coordinate of the vecOrigin of the ent
...
def RCS(client, clientState):
    ...
    while True:
        if win32gui.GetForegroundWindow() == csgoWindow and dbg.read_int(clientState + getOffset(config,'clientStateInGameOffset')) == 6:  # If we are actually playing in game
            localPlayer = dbg.read_int(client + getOffset(config,'localPlayerOffset'))  # Get the localPlayer
            if dbg.read_int(localPlayer + getOffset(config,'shotsFiredOffset')) > 1:  # If we have fired more than 1 shots
                viewAngleX = dbg.read_float(clientState + getOffset(config,'clientStateViewAnglesOffset'))  # Get the X viewAngle
                viewAngleY = dbg.read_float(clientState + getOffset(config,'clientStateViewAnglesOffset') + 0x4)  # Get the Y viewAngle

                aimPunchX = dbg.read_float(localPlayer + getOffset(config,'aimPunchOffset'))  # Get the X aimPunch
                aimPunchY = dbg.read_float(localPlayer + getOffset(config,'aimPunchOffset') + 0x4)  # Get the Y aimPunch
                ...
                dbg.write_float(clientState + getOffset(config,'clientStateViewAnglesOffset'), viewAngleX)
                dbg.write_float(clientState + getOffset(config,'clientStateViewAnglesOffset') + 0x4, viewAngleY)
...
def main():
    ...
    clientState = dbg.read_int(engine + getOffset(config,'clientStateOffset'))  # Get clientState pointer
    st.debug("ClientState : "+ str(clientState))
    st.debug("waiting for LocalPlayer...")
    localPlayer = 0
    while localPlayer == 0:
        localPlayer = dbg.read_int(client + getOffset(config,'localPlayerOffset'))  # Get localPlayer pointer
    ...
    if triggerBotEnabled:
        try:
            thread.start_new_thread(triggerBot,
                                    (client, clientState,))  # Start triggerBot function threaded
        except:
    ...
    if autoBHOPEnabled:
        try:
            thread.start_new_thread(BHOP,
                                    (client, localPlayer, clientState,))  # Start BHOP function threaded
        except:
            st.debug("Could not start bhop thread :(")

    if soundESPEnabled:
        try:
            thread.start_new_thread(soundESP, (client, localPlayer,))  # Start soundESP function threaded
        except:
            st.debug("Could not start soundESP thread :(")

    if rcsEnabled:
        try:
            thread.start_new_thread(RCS, (client, clientState,))  # Start RCS function threaded
        except:
    ...
    while not win32api.GetAsyncKeyState(0x23):  # While END key isn't touched
        inGame = dbg.read_int(clientState + getOffset(config,'clientStateInGameOffset'))
